...
public class PasswordUpdate {
  ...
  /**
   * Nueva contraseña del usuario (debe contener al menos un dígito)
   * @return newPassword
   */
  @NotNull @Pattern(regexp = "^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).*$") @Size(min = 8) 
  @Schema(name = "newPassword", description = "Nueva contraseña del usuario (debe contener al menos un dígito)", requiredMode = Schema.RequiredMode.REQUIRED)
